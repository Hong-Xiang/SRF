...
void MLEM_recon(ReconParameter Reconpara, int Zlength, int *Znum, int *Znuma, float *sinogram, float *volume, int N[3], int *Nchunk, int *Nchunk_acc, int *Itemp, float *SMtemp, float *atan_sino, float *norm_map, int LastIteration, int *IterationsForOutput, char *image_fname);
void RayDrBP_SM(ReconParameter Reconpara, int Zlength, int *Znum, int *Znuma, float *sinogram, float *volume, int N[3], int *Nchunk, int *Nchunk_acc, int *Itemp, float *SMtemp, float *atan_sino, float *norm_map);
void RayDrFP_SM(ReconParameter Reconpara, int Zlength, int *Znum, int *Znuma, float *sinogram, float *volume, int N[3], int *Nchunk, int *Nchunk_acc, int *Itemp, float *SMtemp, float *atan_sino, float *norm_map);
...
int main()
{
	...
	*/
	...
	strcpy(sysmat_fname, "E:\\recon_1_10_2018\\sm\\sm_1_24_2018_2x");
	strcpy(sino_fname, "E:\\Hong_xiang_paper\\recon_2_10_2018\\mm2x_low_float");
	...
	for (seg_ind = 0; seg_ind < Reconpara.Nseg; seg_ind++)
	{
		...
		for (ns = 0; ns < Reconpara.Nsinogram; ns++)
		{
			...
			*/
			...
			fread(SMtemp + Nchunk_acc[seg_ind*Reconpara.Nsinogram + ns], sizeof(float), Nchunk[seg_ind*Reconpara.Nsinogram + ns], fid);
		}
	}
	fclose(fid);
	/*******************************************************************************************************************************/
	...
	for (val = 0; val<4770; val++)
	{
		printf("val=%d\n", val);
		itoa(val, vals, 10);
		//strcat(sino_fname,vals);
		strcpy(image_fname, "E:\\Hong_xiang_paper\\recon_2_10_2018\\recon\\float\\mm2x_low\\mm2x_low.re");
		strcat(image_fname, vals);

	sinogram = (float *)calloc(sizeof(float), Reconpara.Nsinogram*Reconpara.SYM*Zlength);
	...
	fread(sinogram, sizeof(float), Reconpara.Nsinogram*Reconpara.SYM*Zlength, fid_sino);
...
void MLEM_recon(ReconParameter Reconpara, int Zlength, int *Znum, int *Znuma, float *sinogram, float *volume, int N[3], int *Nchunk, int *Nchunk_acc, int *Itemp, float *SMtemp, float *atan_sino, float *norm_map, int LastIteration, int *IterationsForOutput, char *image_fname)
{
	...
	for (niter = 1; niter <= LastIteration; niter++)
	{
		...
		for (n = 0; n<Reconpara.Nsinogram*Reconpara.SYM*Zlength; n++)
		{
			...
			else
			{
				if (tmp_sinogram[n] > realmin)
					tmp_sinogram[n] = float(sinogram[n]) / tmp_sinogram[n];
				else
					tmp_sinogram[n] = float(sinogram[n]) * realmax;
			}
		}
		...
		if (niter == IterationsForOutput[0])
		{
					...
					if (ns_new<(Reconpara.Nsinogram*Reconpara.SYM*Zlength))
					{
						if (sinogram[ns_new]>realmin)
						{
							for (n = 0; n<Nchunk[seg_ind*Reconpara.Nsinogram + ns]; n++)
							{
								...
								{
									volume[Itemp_new] += sinogram[ns_new] * (float(SMtemp[Nchunk_acc[seg_ind*Reconpara.Nsinogram + ns] + n]) );
				...
				for (ns = 0; ns<Reconpara.Nsinogram; ns++)
				{
					...
					if (ns_new<(Reconpara.Nsinogram*Reconpara.SYM*Zlength))
					{
						...
						for (n = 0; n<Nchunk[seg_ind*Reconpara.Nsinogram + ns]; n++)
						{
							...
							if (Itemp_new<Reconpara.Nvolume)
							{
								sinogram[ns_new] += volume[Itemp_new] * (float(SMtemp[Nchunk_acc[seg_ind*Reconpara.Nsinogram + ns] + n]) );
							}
...
void image_blurmap(ReconParameter Reconpara, int Zlength, int *Znum, int *Znuma, int N[3], int *Nchunk, int *Nchunk_acc, int *Itemp, float *SMtemp, float *atan_sino, float *norm_map, float **image_blur)
{
	int ind;
	float *volume;
	float *tmp_sinogram;
