...
class TestBBS(TestCase):
    def get_16module_derenzo_path(self):
        return '/mnt/gluster/Techpi/brain16/recon/lmrec/lm_derenzo'

    def test_16module_dernezo(self):
        cmd = '. '+self.get_16module_derenzo_path()+'/run_lmrec.sh'
        os.system(cmd)
        fortest_result = np.fromfile(self.get_16module_derenzo_path()+'/output_10.rec',np.float32)
