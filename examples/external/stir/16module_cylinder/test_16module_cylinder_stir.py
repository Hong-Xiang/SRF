...
class TestBBS(TestCase):
    def get_16module_cylinder_path(self):
        return '/mnt/gluster/Techpi/brain16/recon/stir/stir_cylinder'

    def test_16module_dernezo(self):
        cmd = '. '+self.get_16module_cylinder_path()+'/run_stir.sh'
        os.system(cmd)
        fortest_result = np.fromfile(self.get_16module_cylinder_path()+'/output_10.v',np.float32)
