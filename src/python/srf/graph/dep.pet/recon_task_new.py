...
from .worker_sino import WorkerGraphBase
...
class ReconstructionTaskBase(MasterWorkerTaskBase):
    ...
    def parse_task(cls, task_spec):
        ...
        result[cls.KEYS.CONFIG.EFFICIENCY_MAP] = {
            ...
            'path_dataset': result['image']['map_dataset']
        }
    ...
    def load_local_data(self, key):
        ...
        if key == self.KEYS.TENSOR.EFFICIENCY_MAP:
            #return np.load('/home/twj2417/SRF/SRF/config/mapnew.npy')
    ...
    def run_task(self):
        ...
        nb_iterations = self.config('recon')['nb_iterations']
        # nb_subsets = self.config('recon')['nb_subsets']
        image_name = self.config('image')['path_file']
        for i in tqdm(range(nb_iterations), ascii=True):
            self._run_with_info(KT.RECON)
            self._run_with_info(KT.MERGE)
            self._print_x()
            self._save_result('{}_{}.npy'.format(image_name, i))
