...
class CutLoRs(Function):
    ...
    def __call__(self, lors):
        ...
        dcos = np.array([p_diff[:, a.value]/p_dis for a in Axis]).T.reshape((-1, 3))

        ratio = np.array(0.5 - (tof(lors)/p_dis))

        p0 = HitOfIndex(0)(lors)
        p1 = HitOfIndex(1)(lors)
        lor_center = np.array([ratio*p_diff[:, a.value] for a in Axis]).T + p0
...
class OnMapByName(Function):
    def __init__(self, names, func):
        self.names = names
        self.func = func

    def __call__(self, v):
        return {n: self.func(v) for n in self.names}


def process_on_axis(a: Axis, lors):
    ...
